...
//
// Stripe se carga SOLO cuando el usuario hace clic en "Comprar".
// Esto elimina los ~247KB de JS de Stripe del bundle inicial
// y los preconnects inutiles que Lighthouse penalizaba.
...
// stripePromise se inicializa solo cuando el usuario hace clic
...
const StoragePlans = () => {
  ...
  const handleSelectPlan = async (plan) => {
    ...
    // Cargamos Stripe y Elements solo en este momento
    if (!Elements) {
      const [sp, { Elements: El }] = await Promise.all([
        getStripePromise(),
          ...
          return (
            <div
              ...
              className={`
                ...
                ${isHighlighted ? 'bg-brand-500/10 border-brand-500/40' : 'bg-card border-border'}
              `}
            >
              {isHighlighted && (
